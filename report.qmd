...
year <- format(as.Date(params$start_date), "%Y")
paste0("In R land ", params_df_R$params.site_name[[1]], "is located at (", params_df_R$params.site_lat[[1]], ", ", params_df_R$params.site_lon, ")")
```
...
print("In python land", params_df_py["params.site_name"][0], "is location at (", params_df_py["params.site_lat"][0], ", ", params_df_py["params.site_lon"][0], " )")
```
...
This report provides an overview of wind resources for `r params$site_name` from `r params$start_date` to `r params$end_date`. This analysis is based on the wind speed at 100 meters dataset within the NREL Wind Toolkit.
...
leaflet(leaflet_df) %>%
  setView(leaflet_df$lon, leaflet_df$lat, zoom = 8) %>%
  addTiles() %>%
...
The graph below shows hourly wind speed for `r params$site_name` and the selected time range. Wind speed values are show in relation to typical turbine cut-in and cut-out speeds.

xxx...update hline to use param values
...
The annual average wind speed for `r params$site_name` was `r annual_average_windspeed_R` m/s. This is `r above_below_R` the value of `r params$req_annual_avg_speed` m/s (13 mph) recommended by the U.S. Energy Information Administration. Wind speed was below the cut-in speed of `r params$cut_in_speed` m/s for `r format(hours_below_cutin_speed_R, big.mark = ",")` hours. Therefore, wind turbines could not operate `r percent_below_cutin_speed_R` percent of the time due to lack of wind. Wind speed exceeded the cut-out speed of `r params$cut_out_speed` m/s for `r format(hours_above_cutout_speed_R, big.mark = ",")` hours. Therefore, wind turbines could not operate `r percent_above_cutout_speed_R` percent of the time due to strong wind. Between `r params$start_date` to `r params$end_date`, wind speeds at this location were within the acceptable operating range of `r params$cut_in_speed` m/s to `r params$cut_out_speed` m/s for `r format(operating_hours_R, big.mark = ",")` hours. Therefore, turbines could operate `r operating_percent_R` percent of the time.
